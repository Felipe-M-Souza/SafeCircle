...
export function createCleaner(): {
  ...
  return {
    sql,
    truncate: async () => {
      await sql`TRUNCATE TABLE outbox_events, audit_events, journey_location_updates, journey_location_sessions, safe_journeys, safety_checkins, alert_location_updates, alert_location_sessions, alert_acknowledgements, push_devices, alert_locations, emergency_alerts, idempotency_keys, group_invitations, group_memberships, trusted_groups, auth_refresh_token_history, auth_sessions, users RESTART IDENTITY CASCADE`;
    },
    close: async () => {
